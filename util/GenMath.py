...
from typing import Union, List, Callable, Any, Protocol, runtime_checkable
import math
import operator
...
def _apply_associative_simplifications(node: BinaryOperator) -> Any:
    ...
    if isinstance(node.lhs, type(node)):
        ...
        if isinstance(node.lhs.rhs, (int, float)) and isinstance(node.rhs, (int, float)):
            if isinstance(node, HasOperatorFunction):
                ...
                raise ValueError(f"Operator {type(node)} does not have an operator function")
        
        # (const1 op A) op const2 -> A op (const1 op const2)
        if isinstance(node.lhs.lhs, (int, float)) and isinstance(node.rhs, (int, float)):
            if isinstance(node, HasOperatorFunction):
    ...
    if isinstance(node.rhs, type(node)):
        ...
        if isinstance(node.lhs, (int, float)) and isinstance(node.rhs.lhs, (int, float)):
            if isinstance(node, HasOperatorFunction):
...
def _apply_binary_numeric(operator: BinaryOperator, lhs: Union[int, float], rhs: Union[int, float]) -> Union[int, float]:
    ...
    if isinstance(operator, HasIdentity) and operator.has_identity:
        if isinstance(operator, IsCommutative) and operator.is_commutative and (lhs == operator.identity_element or rhs == operator.identity_element):
            return lhs if rhs == operator.identity_element else rhs
        elif not isinstance(operator, IsCommutative) and rhs == operator.identity_element:
            return lhs
    
    if isinstance(operator, HasZeroElement) and operator.has_zero_element:
        if isinstance(operator, IsAdditionLike) and operator.is_addition_like and (lhs == operator.zero_element or rhs == operator.zero_element):
            return lhs if rhs == operator.zero_element else rhs
        elif not isinstance(operator, IsAdditionLike) and (lhs == operator.zero_element or rhs == operator.zero_element):
    ...
    if isinstance(operator, HasOperatorFunction):
        ...
        raise ValueError(f"Operator {type(operator)} does not have an operator function")
...
def _apply_commutative_simplifications(node: BinaryOperator) -> Any:
    ...
    if isinstance(node.lhs, (int, float)) and not isinstance(node.rhs, (int, float)):
...
def _apply_constant_folding(node: BinaryOperator) -> Any:
    ...
    if isinstance(node.lhs, (int, float)) and isinstance(node.rhs, (int, float)):
        if isinstance(node, HasOperatorFunction):
...
def _apply_identity_simplifications(node: BinaryOperator) -> Any:
    ...
    if isinstance(node, IsCommutative) and node.is_commutative and (node.lhs == identity or node.rhs == identity):
...
def _apply_unary_numeric(operator: UnaryOperator, operand: Union[int, float]) -> Union[int, float]:
...
def _apply_zero_simplifications(node: BinaryOperator) -> Any:
    ...
    if isinstance(node, IsAdditionLike) and node.is_addition_like and (node.lhs == zero or node.rhs == zero):
        return node.lhs if node.rhs == zero else node.rhs
    
    # 0 op x -> 0 (for multiplication-like operators)
    if not isinstance(node, IsAdditionLike) and (node.lhs == zero or node.rhs == zero):
        return zero
...
def apply_operator(operator: Operator, *operands) -> Any:
    ...
    if len(operands) == 1:
        ...
        raise ValueError(f"Operator {type(operator)} expects 1 or 2 operands, got {len(operands)}.")
